...
d <- tibble(fname = files) %>%
  ...
  map_df(function (x) {  # defines function to run on each data frame
    read_csv(here("SCIP_1B_data_ANON", x$fname)) %>% #need to go up two directories to access this?
      select(ends_with("_1"), -contains("timing")) })
    
    %>% 
      mutate(list = x$id) %>%
      #mutate(listNum = str_c(list,"-",as.character(1:n()))) %>%
...
ms_increasing <- d %>% 
  filter(list == c(1,3,5,7,9,11,13,15)) %>% 
  #group_by(trialType, id) %>%
  group_by(trialName, id) %>%
  summarise(acc = mean(acc, na.rm=TRUE)) %>%
  multi_boot_standard(col = "acc", na.rm = TRUE)
...
ms_decreasing <- d %>% 
  filter(list == c(2,4,6,8,10,12,14,16)) %>% 
  #group_by(trialType, id) %>%
  group_by(trialName, id) %>%
  summarise(acc = mean(acc, na.rm=TRUE)) %>%
  multi_boot_standard(col = "acc", na.rm = TRUE)
...
# df with just INCREASING fam order (odd-numbered lists)
ms <- d %>% 
  filter(list = c(1,2,3,4,9,10,11,12) %>% 
  #group_by(trialType, id) %>%
  group_by(trialName, id) %>%
  summarise(acc = mean(acc, na.rm=TRUE)) %>%
  multi_boot_standard(col = "acc", na.rm = TRUE)
...
# df with just DECREASING fam order (even-numbered lists)
ms_colorB <- d %>% 
  filter(list == c(5,6,7,8,13,14,15,16)) %>% 
  #group_by(trialType, id) %>%
  group_by(trialName, id) %>%
  summarise(acc = mean(acc, na.rm=TRUE)) %>%
  multi_boot_standard(col = "acc", na.rm = TRUE)
#ggplot(ms_decreasing, aes(x = trialType, y = mean, fill = trialType)) + 
ggplot(ms_colorB, aes(x = trialName, y = mean, fill = trialName)) +  
  geom_bar(stat = "identity") +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper)) + 
  geom_hline(yintercept = .5, lty = 3)
```